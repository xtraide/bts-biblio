...
if (empty($tri)) {
	if (empty($search)) { // si la barre de recherceh ne contien rien 
		$sql = "SELECT Titre,Isbn FROM livre WHERE IdGenre=4";/* a changer le nom de la table */
   	}else {/* si la barre de recherche contien des chose  */
		$sql = "SELECT Titre,Isbn FROM livre JOIN editeur e ON e.IdEditeur = livre.IdEditeur JOIN personne p ON p.IdPersonne = livre.IdPersonne WHERE Titre LIKE '%$search%' or Prenom LIKE '%$search%' or Nom LIKE '%$search%'or Annee LIKE '%$search%' AND IdGenre=4; ";
	}
	...
	switch ($tri) {
	case 'Croissant':
		$sql ='SELECT Titre,Isbn FROM livre JOIN editeur e ON e.IdEditeur = livre.IdEditeur JOIN personne p ON p.IdPersonne = livre.IdPersonne WHERE IdGenre=4 ORDER BY titre ASC ;';// requete pour tier par ordre  croisant des titre
		break;
	case 'Décroissant':
		$sql = 'SELECT Isbn,Titre FROM livre JOIN editeur e ON e.IdEditeur = livre.IdEditeur JOIN personne p ON p.IdPersonne = livre.IdPersonne WHERE IdGenre=4 ORDER BY titre DESC;';// requete pour trier par ordre decroisant 
		break;
	}
}
